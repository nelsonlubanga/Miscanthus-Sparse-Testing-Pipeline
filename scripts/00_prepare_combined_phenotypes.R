...
##
## Inputs:
##   ../data/raw/Aberystwyth_raw_phenotypes.csv  -- UK, pre-filtered to the
##       125-genotype panel (extracted from the full ABR33 diversity panel,
##       954 genotypes, sheet "ABR33Data_ForNelson")
##   ../data/raw/Braunschweig_raw_phenotypes.csv -- Germany, pre-filtered to
##       the same 125-genotype panel
...
RAW_UK_CSV    <- "../data/raw/Aberystwyth_raw_phenotypes.csv"
RAW_JKI_CSV   <- "../data/raw/Braunschweig_raw_phenotypes.csv"
...
## -----------------------------------------------------------------------------
## Duplicate-UID and 125-genotype filtering were already applied when this
## extract was prepared from the full 954-genotype institutional file; the
## filters below are kept as a cheap correctness check, not a no-op.
uk_raw <- read.csv(RAW_UK_CSV, stringsAsFactors = FALSE) |>
  filter(!is.na(geno)) |>
...
## As with the UK extract above, genotype filtering and duplicate removal
## were already applied when this extract was prepared; kept here as a
## correctness check.
